...
def str2bool(v):
    """
    Thanks to implementation in ConvNeXt: https://github.com/facebookresearch/ConvNeXt
...
def parse_option():
    parser = argparse.ArgumentParser('MS-MLP training and evaluation script', add_help=False)
...
def main(config):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(config, model, criterion, data_loader_train, optimizer, epoch, mixup_fn, lr_scheduler, model_ema)
        ...
        if config.model_ema and config.model_ema_eval:
            ...
            logger.info(f"Accuracy of the EMA-network on the {len(dataset_val)} test images: {acc1_ema:.1f}%")
